...
namespace AenEnterprise.DataAccess.Repository
{
    ...
    {
        protected readonly AenEnterpriseDbContext _context;
        ...
        public async Task<List<T>> GetListByIdAsync(int id)
        {
            var entityType = typeof(T);
            var property = entityType.GetProperty("UserId");

            if (property == null)
                throw new InvalidOperationException($"Entity {entityType.Name} does not contain a 'UserId' property.");

            return await _context.Set<T>()
                                 .Where(e => (int)property.GetValue(e) == id)
                                 .ToListAsync();
        }
